...
# Species split
species_order = list(genes.keys())
outgroup_species = species_order[:8]
cucurbitaceae_species = species_order[8:]

# Gene list
gene_list = list(next(iter(genes.values())).keys())

# Store data
plot_data = {gene: [] for gene in gene_list}

for species in species_order:
    file_path = f"normalized_count_tables/{species}.txt"
    if not os.path.isfile(file_path):
        print(f"File not found: {file_path}")
        continue

    df = pd.read_csv(file_path, sep="\t")
    df = df.rename(columns={df.columns[0]: "isoform"})
    df["isoform"] = df["isoform"].astype(str)
    long_df = df.melt(id_vars="isoform", var_name="sample", value_name="expression")

    if species not in genes:
        continue

    for gene, isoforms in genes[species].items():
        gene_df = long_df[long_df["isoform"].isin(isoforms)].copy()
        if gene_df.empty:
            continue

        summed = gene_df.groupby("sample")["expression"].sum().reset_index()
        summed["group"] = "Cucurbitaceae" if species in cucurbitaceae_species else "Outgroup"
        summed["gene"] = gene
        plot_data[gene].append(summed)

# Plotting
n_genes = len(gene_list)
fig, axes = plt.subplots(n_genes, 1, figsize=(1, n_genes * 1.2), sharex=True, gridspec_kw={'hspace': 0.04})
palette = {"Outgroup": "#1f78b4", "Cucurbitaceae": "#d95f02"}

for i, gene in enumerate(gene_list):
    ax = axes[i]

    if not plot_data[gene]:
        ax.set_visible(False)
        continue

    df_gene = pd.concat(plot_data[gene], ignore_index=True)

    sns.violinplot(data=df_gene, x="group", y="expression", ax=ax,
                   inner="box", linewidth=0, palette=palette, cut=0)

    # Perform Mann–Whitney U test
    group1 = df_gene[df_gene["group"] == "Cucurbitaceae"]["expression"]
    group2 = df_gene[df_gene["group"] == "Outgroup"]["expression"]
    ...
    # Compute medians
    med_cuc = group1.median()
    med_out = group2.median()

    # Print results to console
    direction = "higher in Cucurbitaceae" if med_cuc > med_out else "higher in Outgroup"
    print(f"{gene}: p={p_value:.3e} ({significance}), "
          f"median Cuc={med_cuc:.3f}, Out={med_out:.3f} → {direction}")

    # Add median text above violins
    y_offset = df_gene["expression"].max() * 0.02  # small offset for spacing
    for j, (grp, median_val) in enumerate(zip(["Outgroup", "Cucurbitaceae"], [med_out, med_cuc])):
        ax.text(j, median_val + y_offset, f"{median_val:.2f}",
                ha='center', va='bottom', fontsize=5, color='black')
    
    # Only add annotation if p is significant
    if significance:
    # Get y-axis position to place line and stars
        y_max = df_gene["expression"].max()
        y_min = df_gene["expression"].min()
        y_max = min(y_max, df_gene["expression"].quantile(0.97))  
        y_range = y_max - y_min
        y_annot = y_max + 0.05 * y_range  # Add padding above the max expression

        # Get the x-axis positions of the two groups
        x1, x2 = 0, 1

        # Draw horizontal line
        #ax.plot([x1, x2], [y_annot, y_annot], lw=0.2, color='black')

        # Draw star(s)
        ax.text((x1 + x2) / 2, y_annot+ 0.02 *y_range, significance,
    ...
    ax.set_ylabel("")
    ax.tick_params(axis='y', which='both', length=0, labelsize=4)
    ax.tick_params(axis='x', which='both', length=0)

    # Set consistent y-limits
    upper_limit = df_gene["expression"].quantile(0.98)
    ax.set_ylim(0, upper_limit*1.5)
    ax.yaxis.set_label_coords(-0.07, 0.5)

        # After all gene plots are drawn
    final_counts = pd.concat(
        [pd.concat(gene_data) for gene_data in plot_data.values() if gene_data],
        ignore_index=True
    )
    group_counts = final_counts["group"].value_counts()
    
    for j, group in enumerate(["Outgroup", "Cucurbitaceae"]):
        n = group_counts.get(group, 0)
        axes[0].text(j, axes[0].get_ylim()[1] * 1.05, f"n={int(n)}", ha="left", va="bottom", fontsize=5, rotation=45)

    # Turn off x-axis labels except for the bottom
    if i < n_genes - 1:
        ax.set_xlabel("")
        ax.set_xticklabels([])
    else:
        ax.set_xlabel("")
        ax.set_xticklabels(["Outgroup", r"$\it{Cucurbitaceae}$"], fontsize=6, rotation=45, ha="right")

    # Draw black border around every subplot
    for side in ['top', 'right', 'bottom', 'left']:
        ax.spines[side].set_color('black')
        ax.spines[side].set_linewidth(0.5)

plt.tight_layout()
#plt.savefig("carotenoid_summarized_10_utest_3.svg", dpi=300)
plt.show()
